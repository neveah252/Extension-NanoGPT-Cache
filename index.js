/* NanoGPT Claude Prompt Cache v1.5.1 (Hover Hotspot, zero-interference)
 * Purpose: Enable NanoGPT Claude prompt caching via cache_control.
 * Default scope: ONLY when API URL contains 'nanogpt' AND model looks like Claude.
 * UI:
 *  - Drawer panel in Extensions.
 *  - Invisible 12x12 hotspot in the corner; hover reveals a small card.
 *    - Hotspot draggable ONLY while holding Alt.
 *    - Uses low z-index and tiny footprint to avoid covering other UI.
 */
...
const defaultSettings = Object.freeze({
  ...
  ttl: '5m',
  onlyClaude: true,
  onlyWhenNanoGPT: true,
  urlSubstring: 'nanogpt',
  hotspotEnabled: true,
  hotspotPos: { x: null, y: null } // from bottom-right in px
});
...
function apiMatchesNanoGPT(apiUrl, substr) {
  ...
  try {
    ...
    return u.hostname.toLowerCase().includes(needle) || u.href.toLowerCase().includes(needle);
  } catch {
    return apiUrl.toLowerCase().includes(needle);
  }
}
...
function buildSettingsContent(container) {
  ...
  container.querySelector('[data-k="hotspotEnabled"]').checked = !!s.hotspotEnabled;
  ...
  container.querySelectorAll('[data-k]').forEach(el => {
    el.addEventListener(el.type === 'checkbox' ? 'change' : 'input', () => {
      ...
      if (key === 'hotspotEnabled') syncHotspotVisibility();
    });
  });
}
...
function ensureHotspot() {
  if (hotspot || card) return;

  hotspot = document.createElement('div');
  hotspot.className = 'ngptc-hotspot';
  hotspot.title = 'NanoGPT Cache Settings (hover)';
  hotspot.setAttribute('aria-label','NanoGPT Cache Settings (hover)');
  ...
  document.body.append(hotspot, card);
  buildSettingsContent(card.querySelector('.ngptc-inner'));

  // position
  positionFromSettings();

  // open/close strictly on hover of hotspot or card
  let hoverCount = 0;
  function open(){ card.classList.add('open'); }
  function close(){ card.classList.remove('open'); }
  hotspot.addEventListener('mouseenter', ()=>{ hoverCount++; open(); });
  hotspot.addEventListener('mouseleave', ()=>{ hoverCount=Math.max(0,hoverCount-1); setTimeout(()=>{ if(hoverCount===0 && !card.matches(':hover')) close(); }, 80); });
  card.addEventListener('mouseenter', ()=>{ hoverCount++; open(); });
  card.addEventListener('mouseleave', ()=>{ hoverCount=Math.max(0,hoverCount-1); if(hoverCount===0) close(); });
  ...
  window.addEventListener('mousemove', (e)=>{
    ...
    hotspot.style.right = r + 'px'; hotspot.style.bottom = b + 'px';
    card.style.right = r + 'px'; card.style.bottom = (b + 10) + 'px';
  });
  window.addEventListener('mouseup', ()=>{
    if(!dragging) return; dragging=false; persistHotspotPos();
  });
...
function positionFromSettings(){
  const s = getSettings();
  const r = (s.hotspotPos?.x == null) ? 8 : s.hotspotPos.x;
  const b = (s.hotspotPos?.y == null) ? 8 : s.hotspotPos.y;
  if (hotspot) { hotspot.style.right = r + 'px'; hotspot.style.bottom = b + 'px'; }
  if (card) { card.style.right = r + 'px'; card.style.bottom = (b + 10) + 'px'; }
}
function persistHotspotPos(){
  const rect = hotspot.getBoundingClientRect();
  const r = Math.max(4, Math.round(window.innerWidth - rect.right));
  const b = Math.max(4, Math.round(window.innerHeight - rect.bottom));
  const s = getSettings(); s.hotspotPos = { x: r, y: b }; saveSettings();
}
function syncHotspotVisibility(){
  const s = getSettings();
  if (hotspot) hotspot.style.display = s.hotspotEnabled ? 'block' : 'none';
  if (!s.hotspotEnabled && card) card.classList.remove('open');
}
